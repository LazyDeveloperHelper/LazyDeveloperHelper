...
def install_requirements(search_path="."):
    for dirpath, _, filenames in os.walk(search_path):
        if REQUIREMENTS_FILE in filenames:
            logger(f"{REQUIREMENTS_FILE} is found!", "success")
            try:
                run(
                    [
                        sys.executable,
                ...
                logger(f"Error installing {REQUIREMENTS_FILE}: {e}", "error")
                return False

        elif ALTERNATIVE_FILE in filenames:
            logger(f"{REQUIREMENTS_FILE} not found, using {ALTERNATIVE_FILE}", "info")
            try:
                run(
                    [
                        sys.executable,
                ...
                logger(f"Error installing {ALTERNATIVE_FILE}: {e}", "error")
                return False
